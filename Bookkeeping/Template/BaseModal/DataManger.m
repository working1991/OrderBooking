...
+(void) saveData:(NSString *)key data:(NSData *)data type:(DataType)type
{
    if( !key || !data ){
        ...
        [MyLog Log:[NSString stringWithFormat:@"DataManager saveData<key=%@,dataLength=>%d>",key,[data length]] obj:NULL];
    }
    ...
    switch ( type ) {
        case Data_DB:
        {
            ...
            if( valueStr && ![valueStr isEqualToString:@""] ){
                ...
                //插入新数据
                NSString *dateStr = [BaseCommon getCurrentDateTime];
        ...
        case Data_File:
        {
            BOOL bSuccess = [data writeToFile:[BaseCommon getSandBoxPath:[MD5 getMD5:key]] atomically:YES];
    ...
    switch ( type ) {
        case Data_DB:
        {
            ...
            if( seconds > 0 ){
                ...
                NSString *dateStr = [BaseCommon getDateStr:date];
        ...
        {
            //先到App所在的沙盒中去找
            NSString *filePath = [BaseCommon getSandBoxPath:[MD5 getMD5:key]];
            ...
            if( [data length] >0 ){
                ...
                NSString *filePath = [BaseCommon getResourcePath:[MD5 getMD5:key]];
